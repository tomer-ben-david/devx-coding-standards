# Swift / SwiftUI Coding Conventions

This document outlines coding standards and best practices for Swift and SwiftUI development.
...
- **No Delegate Storage**: Avoid storing app state in `AppDelegate`. Use it only for system lifecycle events, not as a dependency container.
...
- **Domain-Driven**: Use explicit `struct` types for domain concepts (e.g., `User`, `Order`) instead of primitives or dictionaries.
...
- **Codable**: Prefer `Codable` conformance for data models to simplify logging, debugging, and persistence.
...
- **Structured Logging**: Use `Logger.shared` with canonical metadata keys (e.g., `serviceId`, `outcome`) for critical paths. **No `print()`**.
...
- **Immutability & Finality**: Prefer `let` over `var`. Mark classes as `final` unless inheritance is required. Prefer value types (`struct`) where possible.
